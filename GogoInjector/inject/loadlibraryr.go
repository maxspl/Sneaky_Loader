...
func RVA2OFFSET(dwRva uint32, uiBaseAddress uintptr) (uint32, error) {
	...
	struct_NT_Headers := (*NT_Headers)(unsafe.Pointer(uiExportDir))
	//fmt.Printf("struct_NT_Headers : %T\n", struct_NT_Headers)

	// On additionne l'adresse de OptionalHeader32 avec la taille de ces OptionalHeader32 afin d' arriver a la section suivante
	// qui est Section Headers
	pSectionHeader := uintptr(unsafe.Pointer(&struct_NT_Headers.OptionalHeader32)) + uintptr(struct_NT_Headers.FileHeader.SizeOfOptionalHeader)
...
func GetReflectiveLoaderOffset(lpReflectiveDllBuffer uintptr) (uint32, error) {
	...
	if hex.EncodeToString(b) == "010b" { // check if the dll is 32 bits
		if runtime.GOARCH != "386" { // $env:GOARCH="386" to compile it to 3 bits
			...
			return 0, fmt.Errorf("Gogoloader is :%v. But only 32 bits is supported for the moment.", runtime.GOARCH)
		}
	}
	...
	fmt.Printf("DataDirectory_address :%v\n", DataDirectory_export_adress_hex)
	...
	fmt.Printf("ImageExportDirectory_AddressOfFunctions_address : %v\n", ImageExportDirectory_AddressOfFunctions_address)
